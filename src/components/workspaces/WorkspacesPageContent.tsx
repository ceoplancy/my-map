...
import { isPlatformAdminMetadata } from "@/lib/auth/platformRole"
...
/**
 * 로그인 직후 캐시와 무관하게 목록을 다시 검증한다.
 * (클라이언트 전용 페이지에서만 사용 — `src/pages/workspaces.tsx`의 dynamic ssr:false)
 */
export function WorkspacesPageContent() {
  ...
  /** 카드「관리」와 동일 — 메타데이터 admin 또는 통합 관리자 */
  const isAdmin = isServiceAdmin || isPlatformAdminMetadata(user?.user)
